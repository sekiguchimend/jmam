# 職場改善スコア回答予測システム 開発仕様書

本ドキュメントは、「職場改善スコア回答予測システム」の開発に必要な技術仕様（データベーススキーマ、CSVアップロード仕様、非機能要件）を統合して定義するものです。
...
erDiagram
    responses {
        ...
        text answer_q1 "設問1の回答"
        text answer_q2 "設問2の回答"
        text answer_q3 "設問3の回答"
        text answer_q4 "設問4の回答"
        text answer_q5 "設問5の回答"
        text answer_q6 "設問6の回答"
        text answer_q7 "設問7の回答"
        text answer_q8 "設問8の回答"
        text answer_q9 "設問9の回答"
...
| 回答者ID | `response_id` | `VARCHAR(50)` | | | | CSVの「受注番号」に相当 |
...
| **設問1回答** | `answer_q1` | `TEXT` | | ✅ | | CSV: `【設問ID】　1` |
| ... | `...` | `...` | | ✅ | | 設問9まで同様に定義 |
